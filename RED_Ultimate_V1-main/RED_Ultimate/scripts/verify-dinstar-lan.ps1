...
.EXAMPLE
  .\verify-dinstar-lan.ps1 -DinstarIp 192.168.11.1 -ExpectedAsteriskIp 192.168.11.10
#>
param(
...
$firewallRules = @(Get-NetFirewallRule -DisplayName @('YOUNES DINSTAR *', '*DINSTAR*SIP*', 'Asterisk SIP') -ErrorAction SilentlyContinue)
...
# ── 6. مسبار SIP حي للبوابة (UDP OPTIONS لكل منفذ) ─────────────────────
# قراءة فقط: يثبت أن stack الـ SIP في البوابة حي على 5060+N بلا أي credential.
# فشل المسبار مع نجاح الـ ping يعني غالباً VPN يحجب LAN أو SIP مطفأ في البوابة.
function Test-DinstarSipPort {
    param([string]$Ip, [int]$Port, [string]$LocalIp)
    $udp = $null
    try {
        $udp = New-Object Net.Sockets.UdpClient
        $udp.Client.ReceiveTimeout = 2500
        $localPort = 15000 + $Port
        $udp.Client.Bind((New-Object Net.IPEndPoint([Net.IPAddress]::Any, $localPort)))
        $slot = $Port - 5060
        $body = "OPTIONS sip:$slot@$Ip SIP/2.0`r`n" +
            "Via: SIP/2.0/UDP ${LocalIp}:$localPort;branch=z9hG4bKverify$Port;rport`r`n" +
            "From: <sip:verify@${LocalIp}>;tag=verify$Port`r`n" +
            "To: <sip:$slot@$Ip>`r`n" +
            "Call-ID: verify$Port@$Ip`r`nCSeq: 1 OPTIONS`r`n" +
            "Contact: <sip:verify@${LocalIp}:$localPort>`r`n" +
            "Max-Forwards: 70`r`nContent-Length: 0`r`n`r`n"
        $msg = [Text.Encoding]::ASCII.GetBytes($body)
        $dest = New-Object Net.IPEndPoint([Net.IPAddress]::Parse($Ip), $Port)
        [void]$udp.Send($msg, $msg.Length, $dest)
        $remote = New-Object Net.IPEndPoint([Net.IPAddress]::Any, 0)
        $reply = $udp.Receive([ref]$remote)
        $text = [Text.Encoding]::ASCII.GetString($reply)
        return ($text -match 'SIP/2.0 \d+')
    } catch {
        return $false
    } finally {
        if ($udp) { $udp.Close() }
...
$localIp = ''
if ($managementAddresses.Count -gt 0) { $localIp = $managementAddresses[0].IPAddress }
$sipAlive = @()
if ($localIp) {
    foreach ($p in 5060..5067) {
        if (Test-DinstarSipPort -Ip $DinstarIp -Port $p -LocalIp $localIp) { $sipAlive += $p }
    }
}
if (-not $localIp) {
    $results.Add((New-CheckResult 'dinstar-sip' 'WARN' 'Skipped: no host address on gateway subnet to probe from'))
} elseif ($sipAlive.Count -eq 8) {
    $results.Add((New-CheckResult 'dinstar-sip' 'PASS' "OPTIONS reply on all ports 5060-5067"))
} elseif ($sipAlive.Count -gt 0) {
    $results.Add((New-CheckResult 'dinstar-sip' 'WARN' "OPTIONS reply only on: $($sipAlive -join ',') — check missing port rules/SIMs"))
} else {
    $results.Add((New-CheckResult 'dinstar-sip' 'FAIL' 'No OPTIONS reply on 5060-5067 — VPN may block LAN, or box SIP is down (ping/ARP ok is not enough)'))
}
