<#
.SYNOPSIS
  Bring the proven UC2000 (active gateway IP from .env) online on the dedicated NIC for YOUNES.

.DESCRIPTION
  Keep the gateway on its management NIC. The box has no Wi-Fi: unplugging the
  Realtek cable always disconnects it, so moving it to 192.168.0.x does not help.
  Internet stays on Wi-Fi; DINSTAR stays on the management NIC.

  This script:
    1. Resolves the gateway IP from .env (DINSTAR_IPS / DINSTAR_IP), default 192.168.11.2.
    2. Pings it and finds the Windows address on that NIC (192.168.11.x).
    3. Sets DINSTAR_ENABLED=true in .env (does not change passwords).
    4. Enables IP forwarding so Docker Desktop / WSL can reach the NIC.
    5. Opens the SIP/RTP firewall rules toward the box.
    6. Starts Compose with docker-compose.lan.yml.
    7. Prints the SIP-server IP you must type in enFrame.htm.
...
# Derive the gateway IP from .env (DINSTAR_IPS / first of DINSTAR_IPS) so the script
# never fights the single source of truth; default to the active UC2000 address.
if (-not $DinstarIp -and (Test-Path $EnvFile)) {
    $envContent = Get-Content $EnvFile -Raw
    $ipsMatch = [regex]::Match($envContent, '(?m)^DINSTAR_IPS=(.*)$')
    $ipMatch  = [regex]::Match($envContent, '(?m)^DINSTAR_IP=(.*)$')
    if ($ipsMatch.Success -and $ipsMatch.Groups[1].Value.Trim()) {
        $DinstarIp = $ipsMatch.Groups[1].Value.Trim().Split(',')[0].Trim()
    } elseif ($ipMatch.Success -and $ipMatch.Groups[1].Value.Trim()) {
        $DinstarIp = $ipMatch.Groups[1].Value.Trim()
    }
}
if (-not $DinstarIp) { $DinstarIp = '192.168.11.2' }
