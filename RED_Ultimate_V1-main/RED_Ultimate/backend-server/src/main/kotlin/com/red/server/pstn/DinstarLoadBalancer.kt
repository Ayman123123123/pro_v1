...
class DinstarLoadBalancer(
     ...
     * المنفذ الفيزيائي.
     * وضع البوابة الواحدة: يوجد اسمان حيّان لنفس النظير —
     *  canonical: dinstar-gw-<host-dashes>-port-N (يولّده entrypoint لكل بوابة)
     *  alias:     dinstar-port-N (للجهاز الأول فقط).
     * نستخدم الـ alias لأنه ثابت ولا يعتمد على ترتيب DINSTAR_IPS.
     * ملاحظة أسطول: مع عدة بوابات يجب تمرير اسم الـ canonical المشتق من الـ host.
     */
    private fun pjsipEndpointFor(gateway: DinstarFleetService.Gateway?, portIndex: Int): String {
        ...
        // Asterisk per-port endpoints are named dinstar-port-N (not dinstar-gw-...), verified via `pjsip show endpoints`
        return "dinstar-port-$portIndex"
    }
     ...
     *
     * [callId] يجعل التحرير مقصورًا على صاحب الحجز. بلاه يمسح أي حجز حيّ
     * على المنفذ نفسه ولو كان لمكالمة أخرى (سبب حجز مزدوج بعد كل إنهاء).
     */
    @JvmOverloads
    fun releasePort(gatewayId: UUID?, port: Int, callId: String? = null) {
        ...
        try { reservations?.releasePort(gatewayId, port, callId) } catch (_: Exception) {}
        // مسار البوابة الواحدة يُخزّن gatewayId=null؛ نحرّره أيضًا لكن
        // بنفس قيد callId حتى لا يُمسح حجز مكالمة أخرى.
        if (gatewayId != null) {
            try { reservations?.releasePort(null, port, callId) } catch (_: Exception) {}
        }
    }
