    ...
    fun validate(runtime: Runtime, mongoHost: String?, postgresHost: String?): String? {
        if (runtime == Runtime.DOCKER) {
            if (isLoopback(mongoHost)) {
                return "MONGODB_LOCALHOST_INSIDE_CONTAINER: use db-mongo:27017 (SPRING_MONGODB_URI from docker-compose). localhost inside the JVM is the backend container, not Mongo."
            }
