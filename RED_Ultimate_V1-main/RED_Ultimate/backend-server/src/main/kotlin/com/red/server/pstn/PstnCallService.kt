...
class PstnCallService(
    ...
    fun dial(userId: UUID, suppliedNumber: String, slotIndex: Int? = null, gatewayHost: String? = null): PstnCallResponse {
        ...
        return runCatching {
            ...
            val selection = if (user.pstnGatewayId != null && user.pstnPortIndex != null) {
                ...
                val gatewayId = user.pstnGatewayId
                    ?: throw IllegalStateException("PSTN gateway not assigned for user ${user.redId}")
                val portIndex = user.pstnPortIndex
                    ?: throw IllegalStateException("PSTN port not assigned for user ${user.redId}")
                val permanent = loadBalancer.selectPermanentPort(gatewayId, portIndex, number)
                ...
                // forcedPort/forcedGatewayHost من الـ UI يُحترمان هنا فقط؛ صاحب
                // الربط الدائم أعلاه يخرج من شريحته دائماً.
                loadBalancer.selectPort(number, slotIndex, gatewayHost)
    ...
    fun hangup(userId: UUID, callId: String): PstnHangupResponse {
        ...
        // ⚠️ كان هذا الاستدعاء بلا حماية: غياب مستند المكالمة في Mongo يرفع
        // NoSuchElementException فتُهجَر بقية الدالة — فلا يُحرَّر المنفذ ولا
        // تُسقَط ساق GSM ولا تُنظَّف Redis. الترتيب الصحيح: حرّر أولًا،
        // وسجِّل التاريخ كأثر جانبي.
        val active = resolveActiveCall(userId)
        val ownsThisCall = active?.first == callId
        val a = active
        val port = if (ownsThisCall && a != null) a.second else -1
        if (ownsThisCall && a != null) {
            val gw = a.third.takeIf { it != PstnActiveCallKeys.LOCAL_GATEWAY_ID }
            loadBalancer.releasePort(gw, a.second, callId)
        }
        runCatching { pstn.hangupCall(callId) }
        runCatching { progress.finishByCallId(callId) }
        clearActive(userId)
        runCatching { history.end(callId, user.redId) }
            .onFailure { log.debug("PSTN hangup: history end for {} skipped: {}", callId, it.message) }
        return PstnHangupResponse(callId, port, ownsThisCall)
    }

    /**
     * الرد على مكالمة PSTN واردة (Task 8) — يتحقق من الملكية ثم يُسجَّل
     * الرد في سجل المكالمات. إسقاط الصوت نفسه يتم عبر Asterisk/AMI
     * بأحداث القناة، وهذا المسار يُثبّت الحالة منطقياً فقط.
     */
    fun answer(userId: UUID, callId: String): Map<String, Any> {
        val user = users.findById(userId).orElseThrow { NoSuchElementException("User not found") }
        val owner = findUserByCallId(callId)
        require(owner == null || owner == userId) { "Only the call owner can answer this PSTN call" }
        runCatching { history.answer(callId, user.redId) }
        return mapOf("callId" to callId, "status" to "ANSWERED")
    }

    /**
     * تنظيف حالة المكالمة النشطة للمستخدم.
     *
     * ⚠️ كان يقرأ callId من Redis وحده، فإن كان المفتاح قد مضى (أو حذفه حدث
     * AMI أسبق) بقي صف `pstn_active_calls` في Postgres — و`hasActiveCall`
     * يقرأه، فيُرفَض المستخدم بـALREADY_IN_PSTN_CALL حتى انتهاء المهلة.
     * الآن نسقط إلى Postgres لمعرفة المكالمة المعلَّقة ونحذفها.
     */
    fun clearActive(userId: UUID) {
        val raw = redis.opsForValue().get(activeKey(userId))
        val callId = raw?.split(":")?.firstOrNull()?.takeIf { it.isNotBlank() }
            ?: reservations?.let { r -> runCatching { r.findActiveCallIdByUser(userId) }.getOrNull() }
        ...
        retryScheduler.schedule(
            {
                try {
                    ...
                    val selection = if (user.pstnGatewayId != null && user.pstnPortIndex != null) {
                        val retryGatewayId = user.pstnGatewayId
                            ?: throw IllegalStateException("PSTN gateway not assigned for user ${user.redId}")
                        val retryPortIndex = user.pstnPortIndex
                            ?: throw IllegalStateException("PSTN port not assigned for user ${user.redId}")
                        loadBalancer.selectPermanentPort(retryGatewayId, retryPortIndex, event.phoneNumber)
                    } else {
