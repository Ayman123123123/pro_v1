...
class DinstarConnectionFactory(
    ...
    /**
     * ── لماذا مُرسِل ومَجمَع اتصالات مشتركان ────────────────────────────
     *
     * الجهاز خادم ويب مدمج (Web Server/2.1.0 + MatrixSSL/4.3.0) على معالج
     * ضعيف، ومصافحة TLS عنده تُنفَّذ **بالتسلسل**. القياس الحيّ من داخل
     * حاوية الخادم إلى 192.168.11.1:443:
     *
     *   طلب منفرد        : connect 0.007s | TLS 0.91s | total 1.92s
     *   ستة طلبات متوازية: connect 0.007s | TLS 0.55s → **3.73s** | total 4.89s
     *
     * أي أن TCP فوري دائمًا والتأخير كله في المصافحة. و`connectTimeout` في
     * OkHttp يشمل مصافحة TLS، وكان 5 ثوانٍ. ومع خمس مُجدولات تضرب الجهاز
     * (heartbeat 30s، ابتلاع SMS 12s، تقارير التسليم 60s، CDR 5m، تعلّم
     * الأرقام) يتجاوز زمن المصافحة الحدّ فيسقط الطلب بـ:
     *     WARN DINSTAR query_incoming_sms failed: Connect timed out
     * وهو ما كان يُقرأ خطأً كأن الجهاز غير متصل — والجهاز يرد 200 في
     * الطلب المنفرد في اللحظة نفسها (مُثبت حيًّا 2026-09-06).
     *
     * الإصلاح من الجذر، ثلاث طبقات:
     *  1. `Dispatcher(maxRequestsPerHost=1)` — طلب واحد لكل جهاز في وقت
     *     واحد. لا يُلغي التوازي بين الأجهزة، ويمنع طوابير المصافحة داخل
     *     الجهاز الواحد. (`callTimeout` يحمي من انتظار الطابور للأبد.)
     *  2. `ConnectionPool` مشترك بحياة طويلة — إعادة استخدام الاتصال تجعل
     *     المصافحة تحدث مرة واحدة كل خمس دقائق لا مع كل طلب. keep-alive
     *     مؤكَّد ميدانيًا: طلبان متتاليان على اتصال واحد ينجحان (200/200).
     *  3. مهلة اتصال واقعية (افتراضيًا 15s) تستوعب أسوأ مصافحة قِيست.
     */
    ...
    private fun buildHttpClient(probe: Boolean): OkHttpClient {
        ...
        // الفحص يبقى متوازيًا وبمجمَّع خاص: مسح شبكة كامل لا يجب أن يُسلسل
        // على مُرسِل الإنتاج ولا أن يُلوّث مجمَّعه باتصالات لعناوين ميتة.
        if (!probe) {
            builder.dispatcher(sharedDispatcher).connectionPool(sharedPool)
        }
        ...
        return builder
            ...
            // callTimeout يشمل انتظار الطابور في المُرسِل: مع maxRequestsPerHost=1
            // يجب أن يتّسع لطلب سابق قيد التنفيذ، وإلا صار التسلسل نفسه سببًا
            // للفشل. الحدّ = مهلتان كاملتان.
            .callTimeout(
                if (probe) probeTimeout * 2 else (readTimeout + timeout) * 2,
                TimeUnit.SECONDS
            )
