...
class DinstarFleetController(
    ...
    private fun boundUsersByPort(gatewayId: UUID): Map<Int, com.red.server.auth.model.UserAccount> =
        users.findByPstnGatewayId(gatewayId)
            ...
            .associateBy { it.pstnPortIndex ?: throw IllegalArgumentException("INVALID_CONFIG: pstnPortIndex is null") }
    ...
    fun discover(
        ...
        val actor = runCatching { UUID.fromString(authentication.name) }.getOrNull()
            ?: throw ResponseStatusException(UNAUTHORIZED, "INVALID_PRINCIPAL")
        ...
        return ResponseEntity.ok(mapOf(
            ...
            "found" to found.map {
                mapOf(
                    ...
                    "macAddress" to it.macAddress,
                    "confidence" to it.confidence, "signals" to it.signals,
                    "adoptable" to (it.confidence >= DinstarFleetService.MIN_ADOPT_CONFIDENCE)
                )
            },
    ...
    fun probe(@RequestBody body: Map<String, Any?>): ResponseEntity<Map<String, Any?>> {
        ...
        return ResponseEntity.ok(mapOf(
            ...
            "macAddress" to result.macAddress,
            "confidence" to result.confidence, "signals" to result.signals,
            "adoptable" to (result.confidence >= DinstarFleetService.MIN_ADOPT_CONFIDENCE)
        ))
    }
