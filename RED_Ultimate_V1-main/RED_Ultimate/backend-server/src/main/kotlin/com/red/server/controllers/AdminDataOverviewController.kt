 ...
 *
 * النسخة السابقة كانت تُرجع `0` عند فشل الاستعلام (`getOrDefault(0L)`)، وهذا
 * أخطر من الفشل الصريح: انقطاع Postgres كان يُعرض في لوحة الإدارة كأن المنصة
 * فيها صفر مستخدم وصفر جهاز وصفر بلاغ مفتوح — قراءة تدفع لقرار خاطئ.
 *
 * الآن كل مصدر بيانات (Postgres / Mongo / Redis) يُراقَب عبر [SourceProbe]:
 * أول فشل يُعلَّم المصدر كغير متاح، وتصبح كل مقاييسه `null`، ويظهر في
 * `dataSources` بـ `available=false`. الصفر يعني صفرًا حقيقيًا فقط.
 */
@RestController
...
class AdminDataOverviewController(
    ...
    /**
     * أول مجموعة غير فارغة تُعاد؛ الأسماء المتعددة تغطي هجرات التسمية.
     * عند تعذّر Mongo تُعاد `null` ولا يُفترض صفر.
     */
    ...
    private inner class SourceProbe(
        ...
        fun <T> read(operation: () -> T): T? {
            // بعد أول فشل لا نُثقل مصدرًا ساقطًا بعشرات الاستعلامات.
        ...
        /** لا تُسرَّب رسالة الاستثناء للعميل — رمز ثابت فقط. */
