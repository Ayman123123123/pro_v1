...
CREATE INDEX idx_admin_audit_category ON admin_audit_log(category, created_at DESC);
CREATE INDEX idx_admin_audit_severity ON admin_audit_log(severity, created_at DESC) WHERE severity = 'CRITICAL';
...
CREATE INDEX idx_system_analytics_date ON system_analytics(stat_date DESC);
...
CREATE INDEX idx_system_health_component ON system_health(component, last_check_at DESC);
CREATE INDEX idx_system_health_status ON system_health(status, last_check_at DESC) WHERE status != 'HEALTHY';
...
CREATE INDEX idx_admin_sessions_admin ON admin_sessions(admin_id, is_active, last_active_at DESC);
...
CREATE INDEX idx_feature_flags_enabled ON feature_flags(enabled, expires_at);
...
CREATE TABLE IF NOT EXISTS user_reports (
    ...
    target_content_type VARCHAR(30), -- MESSAGE, POST, STORY, PROFILE, GROUP
    target_content_id VARCHAR(100),  -- ID of the reported content
    category VARCHAR(30) NOT NULL,    -- SPAM, HARASSMENT, HATE_SPEECH, VIOLENCE, SEXUAL, IMPERSONATION, OTHER
    reason TEXT,
    ...
    resolution VARCHAR(30),           -- WARNING_ISSUED, USER_BANNED, CONTENT_REMOVED, NO_ACTION
    admin_notes TEXT,
    resolved_at TIMESTAMP,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT user_reports_category_check CHECK (category IN (
        'SPAM','HARASSMENT','HATE_SPEECH','VIOLENCE','SEXUAL',
        'IMPERSONATION','SCAM','MISINFORMATION','PRIVACY_VIOLATION','OTHER'
    )),
...
CREATE INDEX idx_user_reports_target_user ON user_reports(target_user_id, created_at DESC);
CREATE INDEX idx_user_reports_assigned ON user_reports(assigned_admin_id, status) WHERE status IN ('PENDING','REVIEWING');
...
CREATE INDEX idx_announcements_published ON system_announcements(is_published, priority DESC, show_from);
CREATE INDEX idx_announcements_active ON system_announcements(is_published, show_until) WHERE is_published = TRUE;
...
CREATE INDEX idx_backup_history_completed ON backup_history(completed_at DESC) WHERE status IN ('COMPLETED','VERIFIED');
...
BEGIN
    ...
    SELECT
        ...
        (SELECT COUNT(*) FROM messages WHERE DATE(created_at) = target_date),
        (SELECT COUNT(*) FROM messages WHERE message_type = 'VOICE' AND DATE(created_at) = target_date),
        ...
        COALESCE((SELECT SUM(size_bytes) FROM media_objects), 0)
...
BEGIN
    ...
    FROM users
    UNION ALL
    SELECT 'messages_24h'::VARCHAR(50), COUNT(*)::BIGINT, COUNT(*)::BIGINT, 100.0
