...
on('POST', '/api/admin/content/polls', (_p, _q, b) => {
  const id = uuid();
  // صور الخيارات (نمط X): كائنات {text, imageUrl} أو مصفوفة optionImages الموازية.
  const parallelImages = Array.isArray(b?.optionImages) ? b.optionImages : [];
  const options = (b?.options || []).map((t, i) => ({
    id: uuid(),
    text: typeof t === 'string' ? t : t?.text,
    imageUrl: (typeof t === 'object' && t?.imageUrl) || parallelImages[i] || null,
    votes: 0,
  }));
