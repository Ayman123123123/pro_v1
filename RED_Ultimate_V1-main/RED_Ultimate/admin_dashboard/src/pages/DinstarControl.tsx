    ...
    return (
      <Col xs={24} sm={12} lg={6} xl={6} key={`${gateway.id}-${port.index}`}>
        ...
        >
          ...
          {/* ── الأزرار — أهداف لمس ≥40px: وسط + فجوة 8 ── */}
          <Space size={8} wrap style={{ marginTop: 10, display: 'flex' }}>
            <Button
              type="primary" size="middle" icon={<PhoneOutlined />}
              disabled={!ready}
            ...
            <Button size="middle" icon={<ApiOutlined />} style={{ flex: 1 }} onClick={() => { setUssdTarget({gatewayId: gateway.id, port: port.index}); setUssd(''); }}>
              USSD
            </Button>
            <Button size="middle" icon={<ToolOutlined />} style={{ flex: 1 }} onClick={() => resetPort(gateway.host, port.index)}>
              تهيئة
            </Button>
          </Space>
