...
/**
 * ============================================================================
 * خادم API محاكي للتطوير المحلي فقط — dev-mock-server.mjs
 * ============================================================================
 * لماذا؟ الخادم الحقيقي Spring Boot يحتاج Java/Docker. في بيئات المعاينة
 * الخالية منهما، يرجع بروكسي Vite خطأ 500 لكل طلب /api (بما فيه تسجيل الدخول).
 *
 * ماذا يفعل؟ يقدّم نفس عقود الـ REST التي تستخدمها اللوحة (نفس المسارات والأشكال):
 *   - المصادقة: /api/auth/login (admin/admin123) + refresh + logout
 *   - لوحة القيادة: /api/dashboard/{metrics,charts,health,alerts,realtime}
 *   - المستخدمون: /api/admin/users + الموافقات/الحظر/الرفض (بذاكرة حية)
 *   - SSE: /api/admin/events/stream
 *   - بقية المسارات: استجابات آمنة ([] أو صفحة فارغة أو {success:true})
 *
 * التشغيل:  node dev-mock-server.mjs   (منفذ 8088 — نفس هدف بروكسي Vite)
 * ⚠️ للتجربة والعرض فقط — لا يخزن شيئاً ولا يصل لقواعد بيانات. في الإنتاج
 *    أو مع Docker شغّل الخادم الحقيقي وأوقف هذا (نفس المنفذ).
 * ============================================================================
 */
...
// مسارات تُعاد كمصفوفة فارغة افتراضياً
...
const server = http.createServer(async (req, res) => {
  ...
  // أزل /api وأتقبل /admin اختيارياً (اللوحة تستخدم الشكلين)
  let p = url.pathname.replace(/^\/api\/?/, '');
  p = p.replace(/^master\/v1\//, '');
  const method = req.method.toUpperCase();

  await new Promise((r) => setTimeout(r, 40)); // محاكاة زمن استجابة واقعي
  ...
  if (p === 'auth/me') {
    ...
    return auth.startsWith('Bearer mock-access-')
      ? json(res, 200, adminUser())
      : json(res, 401, { code: 'UNAUTHENTICATED' });
  }
  ...
  // ── المستخدمون ──
  let m;
  if (p === 'admin/users' && method === 'GET') return json(res, 200, pageOf(db.users, url.href));
  if (p === 'admin/users/pending') return json(res, 200, pageOf(db.users.filter((u) => u.status === 'PENDING'), url.href));
  ...
  if ((m = p.match(/^admin\/users\/([^/]+)\/(approve|reject|ban|unban)$/)) && method === 'POST') {
    ...
    if (u) {
      const map = { approve: 'APPROVED', reject: 'REJECTED', ban: 'BANNED', unban: 'APPROVED' };
      u.status = map[m[2]];
    }
    return json(res, 200, { success: true });
  }
  ...
  // ── مجموعات/منشورات/تقارير/محتوى (صفحات الجيل القديم المدمجة) ──
...
server.listen(PORT, '127.0.0.1', () => {
  console.log(`✅ Mock API جاهز على http://127.0.0.1:${PORT} — الدخول: ${ADMIN_USER} / ${ADMIN_PASS}`);
});
