...
```
YNS-XXXX-XXXX      الأبجدية: 23456789ABCDEFGHJKLMNPQRSTUVWXYZ
                   (بلا 0 و1 وI وO منعًا للّبس البصري)
```
...
- `dev-server/db.cjs` — البيانات الأولية تستخدم معرّفات قانونية ثابتة
  (`YNS-7K4M-82QX` للمسؤول)، و`nextRedId()` في `app-routes.cjs` صار ينسخ
  خوارزمية `RedIdGenerator` حرفيًا مع فحص التفرّد.
