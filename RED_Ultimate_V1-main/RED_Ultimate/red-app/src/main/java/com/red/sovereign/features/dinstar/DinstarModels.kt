...
//
// أما بقيّة النماذج فيستهلكها حياً:
// - [DinstarPort]/[DinstarGatewayStatus]/[DinstarFleetStatus] →
//   `DinstarViewModel` + `DinstarAdminScreen` + `settings/PstnConfigScreen`
// - [DinstarIncomingSms]/[DinstarSmsResult]/[DinstarSms] → `DinstarSmsScreen`
// - [DinstarCdr]/[DinstarStatistics]/[DinstarDeviceStatus] → لوحة الإدارة
//
// كل مسارات `/api/admin/dinstar/*` تتطلب دور ADMIN في `SecurityConfig`،
// فشاشات هذه الحزمة تُعرض للمسؤول فقط (يحكمها `TokenStore.role`) —
// لا تُستدعى من جهاز مستخدم عادي فتردّ 403.
//
// أُبقيت هذه النماذج لأنها تصف عقد API البوابة الفعلي كما وثّقته
// «Dinstar GSM Gateway HTTP API»، فهي المرجع الصحيح لكل وصل قادم.
//
...
data class DinstarPort(
     ...
     *
     * كان النص البديل محرَّفًا (`—` = شرطة UTF‑8 قُرئت كـ Latin‑1) فيظهر
     * للمستخدم كرموز مشوّشة مكان «لا قياس».
     */
 ...
 *
 * `gatewayId` أُضيف في التوحيد (2026-09-05): الأسطول يحمل عدة بوابات،
 * وبدونه كان مستحيلاً معرفة أي جهاز استقبل الرسالة.
 * `receivedAt` صار `Long` (epoch ms) بدل نص: النص كان يمنع الترتيب الزمني
 * وتنسيق التاريخ محلياً.
 */
...
/**
 * نتيجة إرسال SMS مجمّع — تبويب «نتائج» في شاشة SMS.
 *
 * يُبنى من استجابة `POST /api/admin/dinstar/sms/send`
 * (status/taskId/queueCount).
 */
data class DinstarSmsResult(
    ...
    /** `SENT_OK` دلالة الجهاز و`ACCEPTED` دلالة الخادم — كلتاهما نجاح. */
...
/**
 * رسالة SMS في سجل الجلسة (صادرة أو واردة) — تعرضها شاشة SMS الإدارية
 * كفقاعات محادثة.
 */
