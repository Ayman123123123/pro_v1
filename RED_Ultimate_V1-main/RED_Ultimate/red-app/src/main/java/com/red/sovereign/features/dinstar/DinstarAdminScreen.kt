...
fun DinstarAdminScreen(viewModel: DinstarViewModel, onBack: () -> Unit) {
    ...
    // تبويبان: مراقبة الأسطول + مركز الرسائل (شاشة SMS المستعادة)
    ...
    Column(Modifier.fillMaxSize().background(Color(0xFF121212))) {
        ...
        TabRow(
            ...
            listOf("الأسطول", "رسائل SMS").forEachIndexed { index, title ->
                Tab(selected = section == index, onClick = { section = index }) {
                    Text(
                        title,
                        fontWeight = if (section == index) FontWeight.Bold else FontWeight.Normal,
                        color = if (section == index) AqyalGold else Color.Gray,
        ...
        when (section) {
            0 -> FleetSection(fleetStatus)
            else -> DinstarSmsScreen(viewModel)
        }
    }
}

@Composable
private fun FleetSection(fleetStatus: DinstarFleetStatus) {
    LazyColumn(
