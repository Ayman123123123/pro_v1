...
/** NOTE: canonical `Channel` lives in ChannelModels.kt (rich model with defaults).
 * This file owns network only — no duplicate data class (fixes duplicate-class
 * build break reported by parallel audit). JSON uses ignoreUnknownKeys so the
 * rich model deserializes from the slim server payload. */
