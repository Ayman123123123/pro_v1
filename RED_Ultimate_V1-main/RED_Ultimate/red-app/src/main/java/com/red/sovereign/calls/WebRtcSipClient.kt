...
class WebRtcSipClient(
    ...
    interface Events {
        ...
        /**
         * وسائط مبكرة (183 Session Progress بـSDP) — القناة الصوتية صارت
         * مفتوحة **قبل** الرد. هذا هو المسار الذي يسمع فيه المتصل ردّ مزوّد
         * الخدمة: «الرقم غير متاح»، «لا يوجد رصيد»، البريد الصوتي، أو أي
         * ردّ آلي من شركة الاتصالات — تمامًا كما يسمعه من هاتف عادي.
         *
         * المستمع يجب أن يُسكت نغمة الرنين المحلية عند هذا الحدث، وإلا
         * غطّت النغمة المحلية على صوت المزوّد.
         */
        fun onEarlyMedia() {}
    }
    ...
    /** محاولة إعادة واحدة عند فشل قابل لإعادة المحاولة (يختار الخادم منفذاً آخر). */
    private var inviteRetried = false
    /** رموز SIP القابلة لإعادة المحاولة التلقائية (لا تشمل 486 مشغول و401/407 مصادقة و404 غير موجود). */
    private val retryableSipCodes = setOf(408, 480, 500, 503, 504)
    ...
    /**
     * ثُبِّتت الوسائط المبكرة من 183 Session Progress.
     *
     * يمنع تثبيت الوصف البعيد مرتين: مرة من 183 ومرة من 200 OK. التثبيت
     * المزدوج يرمي InvalidStateError فيُسقط مكالمة كانت ستنجح.
     */
    private var earlyMediaApplied = false
    ...
    fun invite(targetNumber: String, customHeaders: Map<String, String> = emptyMap()) {
        // حوار جديد ⇒ لا وسائط مبكرة بعد. بلا هذا التصفير تحمل محاولة
        // ثانية على نفس الكائن راية الأولى فيُتخطّى تثبيت SDP في 200 OK.
        earlyMediaApplied = false
        inviteRetried = false
        inviteHeaders = customHeaders
        ...
        when {
            ...
            statusCode == 401 -> {
                ...
                if (challenge != null) {
                    ...
                    val pendingTarget = inviteTarget
                    if (challengedMethod == "INVITE" && pendingTarget != null) {
                        sendInviteWithAuth(pendingTarget, challenge.nonce, challenge.realm)
                    } else {
                    ...
                    if (nonce != null && realm != null) {
                        ...
                        pendingRealm = realm
                        val challengedMethod = parseCSeqMethod(message)
                        val pendingTargetFallback = inviteTarget
                        if (challengedMethod == "INVITE" && pendingTargetFallback != null) {
                            sendInviteWithAuth(pendingTargetFallback, nonce, realm)
                        } else {
            ...
            // ═══ 183 Session Progress — الوسائط المبكرة ═══
            //
            // ⚠️ كان 183 يسقط في `statusCode in 400..699`? لا — كان يسقط في
            // **لا شيء**: لا فرع يطابقه، فيُهمَل بصمت. وهذا يعني أن كل ردّ
            // آلي من مزوّد الخدمة كان يُفقَد:
            //   • «الرقم الذي طلبته غير متاح» — لا يُسمَع.
            //   • «لا يوجد رصيد كافٍ» — لا يُسمَع.
            //   • البريد الصوتي وقوائم IVR — لا تُسمَع ولا يمكن التفاعل معها.
            // المتصل يسمع نغمة رنين محلية مصطنعة حتى تنقطع المكالمة، فيظن
            // أن الشبكة معطّلة والحقيقة أن المزوّد كان يتكلّم.
            //
            // 183 يحمل SDP للطرف البعيد، فيجب تثبيته كوصف بعيد مبكر
            // (`setRemoteDescription` بنوع ANSWER) ليبدأ تدفّق RTP فورًا.
            // ولا يُرسَل ACK: 183 مؤقّت لا نهائي (RFC 3261 §13.2.1) —
            // ACK لا يُرسَل إلا على ردّ 2xx.
            //
            // الحماية من التكرار: 183 قد يتكرّر (أو يتبعه 200 OK بنفس SDP)،
            // وتثبيت الوصف البعيد مرتين يرمي InvalidStateError.
            statusCode == 183 -> {
                val cseqMethod = parseCSeqMethod(message)
                if (cseqMethod == "INVITE") handleEarlyMedia(message)
            }
            ...
            method == "INVITE" -> {
                ...
                val incomingSdp = lastIncomingSdp
                if (incomingSdp != null) {
                    events.onIncomingInvite(incomingSdp, fromNumber)
                } else {
                    android.util.Log.w("SipClient", "STALE_SDP — INVITE without SDP body, ignoring media")
                }
                // Send 180 Ringing
            ...
            // SIP error — مع إعادة محاولة واحدة تلقائياً للأعطال المؤقتة
            // (الخادم يختار منفذ GSM آخر)، ثم تبليغ الواجهة بالرمز النهائي.
            statusCode in 400..699 -> {
                val target = inviteTarget
                if (!inviteRetried && statusCode in retryableSipCodes && target != null) {
                    inviteRetried = true
                    android.util.Log.i("SipClient", "Retrying INVITE once after SIP $statusCode")
                    callId = UUID.randomUUID().toString()
                    invite(target, inviteHeaders)
                } else {
    ...
    /**
     * يعالج 183 Session Progress بـSDP: يفتح مسار الصوت قبل الرد.
     *
     * الأثر العملي: ما يقوله مزوّد الخدمة (رقم غير متاح، لا رصيد، بريد
     * صوتي، قائمة IVR) يصل إلى سمع المستخدم كما لو اتصل من هاتف عادي.
     *
     * SDP في 183 هو **إجابة** على عرضنا في INVITE، فيُثبَّت بنوع ANSWER.
     * ولا نُرسل ACK لأن 183 ردّ مؤقّت.
     */
    private fun handleEarlyMedia(message: String) {
        if (earlyMediaApplied) {
            events.onEarlyMedia()
            return
        }
        val idx = message.indexOf("\r\n\r\n")
        val body = if (idx >= 0) message.substring(idx + 4).trim() else ""
        if (body.isEmpty() || !body.startsWith("v=")) {
            // 183 بلا SDP: تقدّم إشاري فقط (لا وسائط) — يُعامَل كرنين.
            events.onInviteSent()
            return
        }
        toTag = parseToTag(message) ?: toTag
        remoteSdp = body
        earlyMediaApplied = true
        peerConnection.setRemoteDescription(object : org.webrtc.SdpObserver {
            override fun onSetSuccess() {
                android.util.Log.i("SipClient", "Early media (183) applied — carrier audio now flowing")
                events.onEarlyMedia()
            }
            override fun onSetFailure(error: String) {
                // لا نُفشل المكالمة: 200 OK اللاحق سيُثبّت الوصف من جديد.
                earlyMediaApplied = false
                android.util.Log.w("SipClient", "Early media SDP rejected: $error")
            }
            override fun onCreateSuccess(sdp: SessionDescription?) = Unit
            override fun onCreateFailure(error: String) = Unit
        }, SessionDescription(SessionDescription.Type.ANSWER, body))
    }
    ...
    private fun handle200OkToInvite(message: String) {
        ...
        // ⚠️ إن كانت الوسائط المبكرة قد ثُبِّتت من 183، فالوصف البعيد مضبوط
        // أصلًا والحالة `stable`. تثبيته مرة أخرى يرمي InvalidStateError
        // ويُسقط المكالمة عند الرد — أي أن دعم 183 بلا هذا الحارس يكسر
        // المكالمات الناجحة. المطلوب هنا ACK فقط.
        if (earlyMediaApplied) {
            sendAck()
            events.onAnswered()
            return
        ...
        if (answerSdp != null) {
            ...
            android.util.Log.w("SipClient", "STALE_SDP — 200 OK without SDP body, ignoring media")
        }
    }
