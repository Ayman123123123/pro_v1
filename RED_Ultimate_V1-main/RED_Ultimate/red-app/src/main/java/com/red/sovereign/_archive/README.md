...
_تدقيق 2026-09-10: ثلاث شاشات كانت تسكن الأرشيف وهي حيّة (يستوردها
`ui/RedDashboard.kt`) نُقلت إلى بيتها الصحيح `features/calls/` بالحزمة
نفسها (`com.red.sovereign.features.calls`) فبقيت الاستيرادات سليمة:
`CallStatsScreen.kt` (س2733) و`ScheduledCallsScreen.kt` (س2741) و
`CallRecordingsScreen.kt` (س2930). والميتتان فعلًا (`LiveStreamScreen.kt`
و`ConferenceScreen.kt` — صفر مراجع خارج إعلانيهما) أُعيدت تسميتهما إلى
`.kt.archived`. لم يعد في هذا المجلد أي `.kt` يُترجَم داخل الـ APK._
...
### `DeviceSettingsScreen.kt.archived`
- **المسار الأصلي:** `settings/DeviceSettingsScreen.kt` (544 سطرًا)
- **تاريخ الأرشفة:** 2026-08-19
- **السبب:** شاشة إعدادات ثانية موازية للشاشة الحيّة، بلا مستدعٍ.
  وكانت **قائمة ساكنة**: كل بند يستدعي `onNavigate(entry.title)` —
  أي التنقّل بنصّ العنوان المعروض — و`onNavigate` نفسها `= {}`
  افتراضيًّا. فوصلها كان سيعرض تسعة أقسام لا يفعل أيٌّ من بنودها شيئًا.
- **البديل النشط:** `settings/SettingsScreen.kt` عبر `YounesSettingsSheet`
  (مستدعاة من `ui/RedDashboard.kt:467`). وهي تعمل فعلًا: 12 قسمًا
  موصولة بـ`SettingsViewModel` و`DeviceSettingsViewModel`، وقسم
  الأجهزة فيها يحمّل الأجهزة المعتمدة ويلغي اعتمادها ويعرض انتهاء
  الشهادة — بخلاف البنود الساكنة في المؤرشفة.
- **تحقق:** رموزها الستّة كلها محصورة في الملف (صفر مرجع خارجي).

---

### `IncomingPstnCallScreen.kt.archived`
- **المسار الأصلي:** `features/pstn/IncomingPstnCallScreen.kt` (205 أسطر)
- **تاريخ الأرشفة:** 2026-08-19
- **السبب:** شاشة مكالمة واردة من البوابة **بلا مصدر بيانات**. تتبَّعنا
  السلسلة كاملة: `DinstarEventListener` في الخادم يعالج
  `OriginateResponse` و`NewState` و`Hangup` و`Bridge` — وكلها أحداث
  مكالمة **صادرة**؛ ولا حدث وارد يُلتقط، ولا إشارة `PSTN_INCOMING`
  في الخادم أو التطبيق. والأهم أن التوجيه غير ممكن أصلًا: لا حقل هاتف
  في `UserAccount` ولا جدول يربط منفذ البوابة بمستخدم، فلا سبيل لمعرفة
  لمن تُوجَّه المكالمة الواردة.
- **شرط الإحياء:** إضافة ربط (منفذ/رقم ← `redId`) في الخادم، ثم التقاط
  حدث القناة الواردة وبثّه إشارةً عبر `deliverInvite` (فيه صندوق
  انتظار ودفع VoIP جاهزان)، ثم وصل هذه الشاشة بها.
- **ما استُخرج قبل الأرشفة:** `formatPhoneNumber` نُقلت إلى
  `features/pstn/PhoneNumberFormat.kt` لأنها منطق سليم قائم بذاته،
  ويحرسها `PhoneNumberFormatTest` (7 اختبارات).

---

### `DinstarBackoffTest.kt.archived`
- **المسار الأصلي:** `src/test/java/.../features/dinstar/DinstarBackoffTest.kt`
- **تاريخ الأرشفة:** 2026-08-19
- **السبب:** كان يحرس منحنى إعادة الاتصال في `DinstarWebSocketBridge`،
  وقد أُرشف ذلك الملف. والاختبار **يستنسخ** الصيغة محليًّا ولا
  يستوردها (لأن الأصل خاصّ ويعتمد على OkHttp وAndroid Log)، فهو ما
  زال يُترجَم وينجح — لكنه صار يختبر نسخته الخاصة من حسابٍ لم يعد له
  أصل في الشيفرة. اختبارٌ لا يحرس شيئًا أسوأ من غيابه: يوحي بتغطية
  غير موجودة.
- **إن أُعيد `DinstarWebSocketBridge` يومًا فأعِد هذا الاختبار معه.**
- **ما بقي:** `DinstarOperatorMappingTest.kt` لم يُمسّ — يحرس
  `YemenOperator` وهي حيّة ومستعملة في 19 موضعًا.
