...
class StoryViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun upload(uri: Uri, caption: String? = null, visibility: String = "CONTACTS", mediaType: String? = null) = viewModelScope.launch {
        ...
        when (val uploaded = media.upload(uri)) {
            ...
            is ApiResult.Success -> when (val created = client.request("POST", "/api/stories", json.encodeToString(CreateStoryRequest(uploaded.value.objectKey, caption, visibility, mediaType = mediaType ?: uploaded.value.mimeType)))) {
    ...
    fun createTextStory(text: String, backgroundColor: String = "#1565C0", visibility: String = "CONTACTS") = viewModelScope.launch {
        ...
        when (val created = client.request("POST", "/api/stories", json.encodeToString(CreateStoryRequest("text://${text.hashCode()}", text, visibility, mediaType = "TEXT", backgroundColor = backgroundColor)))) {
    ...
    fun createVoiceStory(uri: Uri, durationMs: Long, waveform: List<Int>, visibility: String = "CONTACTS") = viewModelScope.launch {
        state = StoryState.Uploading
        when (val uploaded = media.upload(uri)) {
            is ApiResult.Error -> state = StoryState.Error(uploaded.message)
            is ApiResult.Success -> when (val created = client.request("POST", "/api/stories", json.encodeToString(CreateStoryRequest(uploaded.value.objectKey, null, visibility, mediaType = "VOICE", durationMs = durationMs, waveform = waveform)))) {
                is ApiResult.Success -> runCatching { json.decodeFromString<Story>(created.value) }.onSuccess { stories.add(0, it); state = StoryState.Idle }
                is ApiResult.Error -> state = StoryState.Error(created.message)
            }
        }
