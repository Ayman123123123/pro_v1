...
/**
 * 📨 رسالة SMS واردة من الجهاز — كانت موجودة في النسخ القديمة
 * (DinstarSmsScreen تبويب «واردة») وفُقدت عند الدمج.
 *
 * تصل في الزمن الحقيقي عبر WS (DINSTAR_SMS) أو تُجلب من
...
/**
 * نتيجة إرسال SMS مجمّع — تبويب «نتائج» في الشاشة القديمة.
 * يُبنى من استجابة POST /api/admin/dinstar/sms/send
 * (status/taskId/queueCount) ثم تُحدَّث من /result إن توفر.
 */
data class DinstarSmsResult(
    ...
    /** "SENT_OK" في الشاشة القديمة — التوافق مع نفس دلالة النجاح. */
