...
class LocalServerDiscovery(private val context: Context) {
    ...
    private fun knownBases(): LinkedHashSet<String> {
        ...
        val candidateSeeds = runCatching {
            BuildConfig.RED_SERVER_CANDIDATES.split(',').map { it.trim() }.filter { it.isNotBlank() }
        }.getOrDefault(emptyList())
        val gatewaySeed = getGatewayIp()?.let { "http://$it:8088" }

        val seeds = (listOfNotNull(
            ServerEndpoint.url(),
            BuildConfig.RED_SERVER_URL,
            gatewaySeed,
            "http://192.168.0.42:8088",
            "http://192.168.137.1:8088",
            "http://10.0.2.2:8088",
            "http://192.168.1.50:8088"
        ) + candidateSeeds).distinct()
    ...
    private fun lanBases(): LinkedHashSet<String> {
        ...
        interfaces.filter { runCatching { it.isUp && !it.isLoopback }.getOrDefault(false) }.forEach { network ->
            network.interfaceAddresses.forEach { binding ->
                ...
                val self = bytes[3]
                val octets = linkedSetOf(1, 2, 42, 50, 100, 101, 254, self)
