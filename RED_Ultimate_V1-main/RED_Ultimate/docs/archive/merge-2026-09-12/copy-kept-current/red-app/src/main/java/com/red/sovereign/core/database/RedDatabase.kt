    ...
    companion object {
        ...
        private fun buildDatabase(context: Context): RedDatabase {
            ...
            fun create(): RedDatabase = Room.databaseBuilder(
                ...
                .addMigrations(REACTION_MIGRATION_1_2, INDEX_MIGRATION_2_3, STORY_MIGRATION_3_4)
...
/** حفظ بيانات الحالة التي يحتاجها عارض النص/الصوت بعد إعادة فتح التطبيق. */
private val STORY_MIGRATION_3_4 = object : androidx.room.migration.Migration(3, 4) {
    override fun migrate(database: androidx.sqlite.db.SupportSQLiteDatabase) {
        database.execSQL("ALTER TABLE `stories` ADD COLUMN `ownerUsername` TEXT NOT NULL DEFAULT ''")
        database.execSQL("ALTER TABLE `stories` ADD COLUMN `ownerDisplayName` TEXT NOT NULL DEFAULT ''")
        database.execSQL("ALTER TABLE `stories` ADD COLUMN `backgroundColor` TEXT")
        database.execSQL("ALTER TABLE `stories` ADD COLUMN `durationMs` INTEGER")
        database.execSQL("ALTER TABLE `stories` ADD COLUMN `waveformJson` TEXT NOT NULL DEFAULT ''")
        database.execSQL("ALTER TABLE `stories` ADD COLUMN `visibleTo` TEXT NOT NULL DEFAULT 'EVERYONE'")
        database.execSQL("ALTER TABLE `stories` ADD COLUMN `audienceJson` TEXT NOT NULL DEFAULT ''")
    }
}
