...
|---|---|---|
| تجميع Android الأصلي | `:app:testDebugUnitTest` ناجح في شجرة العمل الأصلية | معلق |
...
| مراجعة الفروق | تصنيف الفروق الكبيرة | موثق |
...
يُعدّ المشغل alias مؤقتاً داخل حاوية `red-storage` باستخدام credentials البيئة الداخلية فقط، ثم يعكس buckets التطبيق إلى وجهة backup معتمدة ومشفرة. لا تستخدم عملية mirror للوصول إلى أي وسيط خارجي غير مخزن الكائنات.
