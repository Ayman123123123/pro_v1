...
class DinstarCdrMappingTest {
    ...
    fun placeholderCountMatchesColumnCount() {
        ...
        // ⚠️ كان هذا `assertEquals(14, columns.size)` — رقم سحري تجاوزه
        // المخطَّط عند إضافة `end_time` و`raw_data` (V45)، فصار الاختبار
        // يفشل على جملة صحيحة تمامًا ويحجب الأخطاء الحقيقية.
        // القائمة الصريحة تكشف أي انزلاق في الترتيب أيضًا — وهو ما يهمّ
        // فعلًا لأن الوسائط تُمرَّر يدويًا بالموضع في CdrIngestScheduler.
        assertEquals(
            listOf(
                "gateway_id", "call_id", "port_index", "start_time", "answer_time",
                "duration_seconds", "ring_duration_seconds", "direction", "status",
                "caller_number", "callee_number", "hangup_cause", "gsm_code", "codec",
                "end_time", "raw_data"
            ),
            columns,
            "أعمدة الإدراج تغيّرت: حدّث ترتيب الوسائط في CdrIngestScheduler و DinstarHardwareService معًا"
        )
        assertEquals(
