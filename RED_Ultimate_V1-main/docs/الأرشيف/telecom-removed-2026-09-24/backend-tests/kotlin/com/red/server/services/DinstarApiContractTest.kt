...
class DinstarApiContractTest {
    ...
    // ── مدموج من com.red.server.DinstarApiContractTest (أُلغي التكرار) ────
    // 32 هو حد query_sms_result لا send_sms — الخلط كان يرفض دفعات مشروعة.

    @Test
    fun `recipient limit is 128 not 32 merged`() {
        assertEquals(128, DinstarHardwareService.MAX_SMS_RECIPIENTS)
    }

    @Test
    fun `message text limit is 1500 bytes merged`() {
        assertEquals(1500, DinstarHardwareService.MAX_SMS_TEXT_BYTES)
    }

    @Test
    fun `limit measured in bytes not chars merged`() {
        val arabic = "س".repeat(800)
        assertEquals(1600, arabic.toByteArray(Charsets.UTF_8).size)
        assertTrue(arabic.toByteArray(Charsets.UTF_8).size > DinstarHardwareService.MAX_SMS_TEXT_BYTES)
        assertTrue(arabic.length < DinstarHardwareService.MAX_SMS_TEXT_BYTES)
    }
