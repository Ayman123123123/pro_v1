    ...
    fun checkAndTriggerLearning() {
        ...
        for (gw in gateways) {
            ...
            for (port in ports) {
                ...
                // كل المشغلين — القاعدة من خريطة المشغل لا Sabafon فقط.
                // يمن موبايل CDMA/LTE لها رموزها؛ الفارغ = تخطَّ بأمان.
                val rule = com.red.server.services.DinstarHardwareService.learningRuleFor(operator)
                if (rule == null) {
                    log.debug("Port {} on {} operator {} has no learning rule — skipping", index, gw.host, operator)
                    continue
                }
                ...
                log.info(
                    "Port {} on {} ({}) is missing its number — attempt {}/{} via {}:{}",
                    index, gw.host, operator, (prior?.count ?: 0) + 1, maxAttemptsPerPort,
                    rule.destination, rule.text
                )
                val success = hardware.triggerNumberLearning(
                    port = index, host = gw.host,
                    destination = rule.destination, text = rule.text
                )
