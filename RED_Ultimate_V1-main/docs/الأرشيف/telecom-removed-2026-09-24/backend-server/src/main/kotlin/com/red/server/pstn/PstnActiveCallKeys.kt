...
object PstnActiveCallKeys {
    ...
    private fun gatewayId(raw: String): UUID = when (raw.lowercase()) {
        // `unknown` تكتبه PstnBridgeController في `red:pstn:callport:*` عند
        // غياب معرّف البوابة. إسقاطها كان يجعل parse ترجع null فيُفقد المنفذ.
        "local", "null", "configured", "unknown", "" -> LOCAL_GATEWAY_ID
