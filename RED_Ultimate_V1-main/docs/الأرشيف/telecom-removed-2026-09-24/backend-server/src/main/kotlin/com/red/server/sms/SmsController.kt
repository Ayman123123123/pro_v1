 ...
 * - DELETE /api/sms/conversation/{n}          → حذف محادثة كاملة بعملية ذرية (يغني عن N طلبات)
...
class SmsController(private val sms: SmsService) {
    ...
    /**
     * حذف محادثة كاملة بطلب واحد ذري — يغلق TODO العميل
     * (SmsViewModel.deleteConversation كان يحذف N رسالة فرادى).
     * العميل الحديث يستخدمه أولاً ويسقط للـ chunked القديم عند 404
     * (خادم قديم لم يُحدَّث بعد).
     */
    @DeleteMapping("/conversation/{number}")
    fun deleteConversation(
        ...
        val userId = UUID.fromString(authentication.name)
        return try {
            val deleted = sms.deleteConversation(userId, number)
            ResponseEntity.ok(mapOf("ok" to true, "deleted" to deleted))
        } catch (e: IllegalArgumentException) {
            ResponseEntity.badRequest().body(mapOf("error" to (e.message ?: "INVALID_NUMBER")))
        }
    }
