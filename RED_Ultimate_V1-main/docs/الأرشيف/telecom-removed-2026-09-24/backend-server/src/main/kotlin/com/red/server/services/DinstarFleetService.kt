...
class DinstarFleetService(
    ...
    companion object {
         ...
         */
        const val MIN_ADOPT_CONFIDENCE = 70
    }
        ...
        jdbc.update(
            """INSERT INTO telecom_gateways
               ...
               VALUES (?,?,'DINSTAR',?,?,?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)
                -- المعرّف مشتق من التسلسلي عند توفره وإلا من (host,api_port)، فيتغير
                -- بين الإقلاعات (بذرة بلا تسلسلي ثم bootstrap مع تسلسلي). الهدف الآمن
                -- هو (host,api_port) مع الحفاظ على id القائم لحماية FK users.pstn_gateway_id.
                ON CONFLICT (host, api_port) DO UPDATE SET
    ...
    private fun inferModel(
        ...
        // يمن موبايل CDMA2000/EVDO/LTE — ليست GSM؛ أي منها يعني متغيّر -T
        val hasLte = radios.any {
            it.contains("LTE") || it.contains("4G") || it.contains("CDMA") ||
                it.contains("EVDO") || it.contains("EV-DO") || it.contains("1X") ||
                it.contains("EHRPD") || it.contains("WCDMA") || it.contains("VOLTE")
        }
