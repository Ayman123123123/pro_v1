...
class EnhancedPstnManager(
    ...
    /**
     * ينهي كل قناة SIP حيّة تخصّ منفذًا خلويًا بعينه.
     *
     * ضرورية لإصلاح «البورت المعلّق»: عندما لا نعرف callId (مات التطبيق،
     * أو ضاعت مفاتيح الكاش، أو تعلّق المنفذ بعد إعادة تشغيل الخادم) يبقى
     * تحرير الحجز منطقيًا فقط والجهاز يرى المنفذ مشغولًا. هنا نُسقط الساق
     * على أستيريكس نفسه، فتصل BYE إلى البوابة ويعود المنفذ حرًّا فعلًا.
     *
     * المطابقة على اسم النظير المشتق من المنفذ — `…-port-N` أو `dinstar-port-N`
     * أو حساب المنفذ `ymnNN` إن كان معروفًا — لأن اسم قناة PJSIP يحمل اسم
     * النظير: `PJSIP/dinstar-gw-192-168-11-1-port-4-00000012`.
     *
     * @return عدد القنوات التي أُسقطت.
     */
    fun hangupPortChannels(gatewayId: UUID?, port: Int): Int {
        if (port !in 0..63) return 0
        val conn = getConnection() ?: return 0
        val channels = try {
            val cmd = org.asteriskjava.manager.action.CommandAction("core show channels concise")
            val response = conn.connection.sendAction(cmd, actionTimeoutMs)
            val lines = when (response) {
                is org.asteriskjava.manager.response.CommandResponse -> response.result ?: emptyList()
                else -> emptyList()
            }
            // الصيغة المختصرة: channel!context!exten!… — الحقل الأول كافٍ.
            lines.mapNotNull { line -> line.substringBefore('!').takeIf { it.startsWith("PJSIP/") } }
        } catch (e: Exception) {
            log.debug("core show channels failed while releasing port {}: {}", port, e.message)
            markUnhealthy(conn)
            return 0
        }
        // اللاحقة `-port-N` يجب أن تُطابق الرقم كاملًا: `-port-1` لا يُطابق
        // `-port-11`. اسم القناة يُلحق عدّادًا بشرطة، فالحدّ هو `-`.
        val portSuffix = Regex("(?:^|-)port-$port(?:-[0-9a-fA-F]+)?$")
        val victims = channels.filter { ch ->
            val peer = ch.removePrefix("PJSIP/").substringBefore(';')
            portSuffix.containsMatchIn(peer)
        }
        var hung = 0
        for (ch in victims) {
            if (hangupChannel(ch)) hung++
        }
        if (hung > 0) {
            log.warn("Force-released DINSTAR port {} on gateway {}: hung up {} channel(s)", port, gatewayId ?: "local", hung)
        } else if (victims.isNotEmpty()) {
            log.warn("Force release of port {} matched {} channel(s) but none could be hung up", port, victims.size)
        }
        return hung
    }
