...
class DinstarStatusSnapshotService(
    ...
    @org.springframework.beans.factory.annotation.Value("\${red.dinstar.enabled:false}") private val dinstarEnabled: Boolean = false

    @PostConstruct
    fun start() {
        if (!dinstarEnabled) {
            log.info("DINSTAR disabled — snapshot scheduler not started")
            return
        }
        refreshNow()
