...
class DinstarWebSocketHandler(
    ...
    /**
     * بث نتيجة إرسال SMS لجميع العملاء.
     * يُستدعى من DinstarApiService بعد send_sms.
     */
    fun broadcastSmsSent(gatewayId: String, result: Map<String, Any?>) {
        broadcastEvent("DINSTAR_SMS_SENT", mapOf("gatewayId" to gatewayId, "result" to result))
    }

    /**
     * بث نتيجة الاستعلام عن إرسال SMS لجميع العملاء.
     */
    fun broadcastSmsResult(gatewayId: String, result: Map<String, Any?>) {
        broadcastEvent("DINSTAR_SMS_RESULT", mapOf("gatewayId" to gatewayId, "result" to result))
    }

    /**
     * بث حالة تسليم SMS لجميع العملاء.
     */
    fun broadcastSmsDeliveryStatus(gatewayId: String, result: Map<String, Any?>) {
        broadcastEvent("DINSTAR_SMS_DELIVERY", mapOf("gatewayId" to gatewayId, "result" to result))
    }

    /**
     * بث SMS واردة مع معرف البوابة لجميع العملاء.
     */
    fun broadcastIncomingSms(gatewayId: String, result: Map<String, Any?>) {
        broadcastEvent("DINSTAR_SMS", mapOf("gatewayId" to gatewayId) + result)
    }

    /**
     * بث إيقاف مهمة إرسال SMS لجميع العملاء.
     */
    fun broadcastSmsTaskStopped(gatewayId: String, taskId: Int) {
        broadcastEvent("DINSTAR_SMS_TASK_STOPPED", mapOf("gatewayId" to gatewayId, "taskId" to taskId))
    }

    /**
     * بث نسخة احتياطية للإعدادات لجميع العملاء.
     */
    fun broadcastConfigBackup(gatewayId: String, backup: Map<String, Any?>) {
        broadcastEvent("DINSTAR_CONFIG_BACKUP", mapOf("gatewayId" to gatewayId, "backup" to backup))
    }

    /**
     * بث استعادة الإعدادات لجميع العملاء.
     */
    fun broadcastConfigRestore(gatewayId: String, backupId: UUID?, result: Map<String, Any?>) {
        broadcastEvent("DINSTAR_CONFIG_RESTORE", mapOf("gatewayId" to gatewayId, "backupId" to backupId?.toString(), "result" to result))
    }
