...
@Serializable
data class IceConfigurationDto(val expiresAt: Long, val iceServers: List<IceServerDto>)
...
class WebRtcEngine(
    ...
    suspend fun create(video: Boolean): ApiResult<Unit> {
        ...
        peer = factory.createPeerConnection(config, observer)
            ?: return ApiResult.Error(null, "PEER_CONNECTION_FAILED")

        val constraints = MediaConstraints().apply {
            mandatory.add(MediaConstraints.KeyValuePair("OfferToReceiveAudio", "true"))
            if (video) mandatory.add(MediaConstraints.KeyValuePair("OfferToReceiveVideo", "true"))
        }
    ...
    fun answer() {
        ...
        peer?.createAnswer(sdpObserver(setLocal = true), constraints)
    }

    fun setRemote(description: SessionDescription, after: (() -> Unit)? = null) =
        peer?.setRemoteDescription(sdpObserver(after = after), description)
    ...
    fun release() {
        ...
        localMedia?.audioTrack?.dispose()
        localMedia?.videoTrack?.dispose()
        audioSource?.dispose()
        videoSource?.dispose()
        peer?.close()
        peer?.dispose()
    ...
    private suspend fun loadIce(): IceConfigurationDto? = withContext(Dispatchers.IO) {
        ...
        when (val response = client.request("GET", "/api/calls/ice-servers")) {
            is ApiResult.Success -> runCatching {
                json.decodeFromString<IceConfigurationDto>(response.value)
            }.getOrNull()
    ...
    private fun camera(enumerator: CameraEnumerator): VideoCapturer? {
        enumerator.deviceNames.firstOrNull(enumerator::isFrontFacing)?.let {
            enumerator.createCapturer(it, null)?.let { camera -> return camera }
        }
        return enumerator.deviceNames.firstNotNullOfOrNull { enumerator.createCapturer(it, null) }
    }

    private fun sdpObserver(setLocal: Boolean = false, after: (() -> Unit)? = null): SdpObserver =
        object : SdpObserver {
    ...
    private val observer = object : PeerConnection.Observer {
        ...
        override fun onIceConnectionChange(state: PeerConnection.IceConnectionState) {
            // Map old-state observer to new-state for UI decisions.
            ...
            events.onConnectionState(mapped)
        }
        ...
        override fun onAddTrack(receiver: RtpReceiver, streams: Array<out MediaStream>) {
            (receiver.track() as? VideoTrack)?.let(events::onRemoteVideo)
        }
