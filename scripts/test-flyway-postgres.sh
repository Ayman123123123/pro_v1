#!/usr/bin/env bash
# Verify Flyway's fresh-install selection and SQL against disposable PostgreSQL.
# CI runs this against the deployed 16.9 and PostgreSQL 17. B64 must be selected
# instead of the broken historical V1..V64 upgrade chain.
...
# Keep the client matching the service; PG17's psql client alone cannot prove
# the deployed PG16 schema works. CI supplies both variables from its matrix.
POSTGRES_CLI_IMAGE="${POSTGRES_CLI_IMAGE:-postgres:17-alpine}"
POSTGRES_EXPECTED_MAJOR="${POSTGRES_EXPECTED_MAJOR:-17}"
...
psql_ci() {
  docker run --rm --network host \
    -e PGPASSWORD -e PGDATABASE -e PGUSER "$POSTGRES_CLI_IMAGE" \
    psql -X -h 127.0.0.1 -v ON_ERROR_STOP=1 "$@"
}

server_version_num=$(psql_ci -tAc 'SHOW server_version_num')
server_major=$((server_version_num / 10000))
if [ "$server_major" != "$POSTGRES_EXPECTED_MAJOR" ]; then
  echo "FAIL: connected to PostgreSQL $server_major, expected $POSTGRES_EXPECTED_MAJOR" >&2
  exit 1
fi
flyway migrate
...
printf 'PASS: Flyway B64 installs %s application tables on PostgreSQL %s; no retired PSTN tables; second migrate validates\n' "$count" "$server_major"
