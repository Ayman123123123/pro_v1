...
class LiveStreamServiceTest {
    ...
    fun `starting a stream registers broadcaster and returns zero viewers`() {
        val stream = service.startStream("stream-1", "YNS-1234-5678")

        assertEquals("stream-1", stream.streamId)
        assertEquals("YNS-1234-5678", stream.broadcasterId)
    ...
    fun `starting the same stream twice keeps a single entry`() {
        service.startStream("stream-1", "YNS-1234-5678")
        service.startStream("stream-1", "RED-AAAA-BBBB") // لا يستبدل الموجود

        val active = service.getActiveStreams()
        assertEquals(1, active.size)
        assertEquals("YNS-1234-5678", active.first().broadcasterId)
    }
